#This program modifies some of the functions in the mnlogit package to 
#allow for fitting of MNL models with varying choice sets (i.e.,
#for each observation a varying subset of offers is observed).
#
#Most code below is derived from the mnlogit package source, 
#https://cran.r-project.org/web/packages/mnlogit/index.html,
#which is under the GPL-v2 license with the following copywright:
#
#Copyright (C) 2016 Asad Hasan and Zhiyu Wang and Alireza S. Mahani
#
#The modifications to the mnlogit code to accommodate varying choice sets are under
#the GPL-v2 license with the copywright below. These modifications were performed by
#Ryan McNellis on March 29th 2020.
#
#Copyright (C) 2020 Ryan McNellis
#
#Due to package import issues, we also copy/paste the Solve function from the
#limSolve package (https://cran.r-project.org/web/packages/limSolve/index.html). 
#The Solve function under the limSolve package is under the GPL-v2 license with the following copywright:
#
...
library("Formula")
#library("pracma")
#The Solve function below is taken verbatim from the limSolve package.
#This is the only function taken from the limSolve package.
Solve <- function (A, B = diag(nrow = nrow(A)), tol = sqrt(.Machine$double.eps)) 
{
  M <- ginv(A, tol)
  if (is.null(M)) 
    return(NULL)
